...
Installing a pattern writes only its knowledge bundle — `.patterns/<name>/` (arch.yaml + structure/rules/recipes/adrs) and the root `AGENTS.md` router. It does **not** scaffold source folders or files; the user's `src/` is untouched. A pattern is a *map* the agent reads to know where code belongs, not a template that creates code.
